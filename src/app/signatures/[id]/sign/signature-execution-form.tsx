...
// Step 5a.2 — approved, factual final-action wording. Deliberately makes no
// claim about legal effect beyond what the product and the reviewed
// consent language (rendered above this form) actually establish.
...
// Long enough for the success confirmation to be genuinely perceivable
// (and screen-reader-announced via role="status") before navigating away —
// short enough not to feel like a stall. Deterministic (no polling, no
// retry), matching the "no toast-only success path" requirement: this is
// the required inline state, the delay only paces the redirect after it.
...
// Pure UI client of executeStaffSignature (Step 5a.1) — this component
// introduces no signing business logic of its own. Every authorization,
// status-precondition, applicability, and integrity check happens
// server-side inside that action; client-side validation here only
// improves the UX for the two cases a user can trivially self-correct
// (blank name, unchecked consent) and is never treated as the security
// boundary.
export function SignatureExecutionForm({ requestId, expectedSignerName }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    // — the Button below is also disabled while loading or after success,
    // but this is the actual, authoritative client-side guard.
    ...
    setLoading(true)
    const result = await executeStaffSignature(requestId, { signerName, consentAccepted })
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Complete Your Signature</CardTitle>
      </CardHeader>
      <form onSubmit={handleSubmit} noValidate>
        ...
        <CardFooter className="flex items-center justify-between gap-3">
          <Link href={`/signatures/${requestId}`} className="text-sm text-surface-500 hover:text-surface-700">
            Cancel
          </Link>
