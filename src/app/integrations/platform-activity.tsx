...
import type { getAuditEvents } from "@/lib/actions/audit"

type EventRow = Awaited<ReturnType<typeof getAuditEvents>>["events"][number]
...
export function PlatformActivityCard({ events }: { events: EventRow[] }) {
  return (
    <Card>
      <CardHeader className="flex-row items-center justify-between">
        <CardTitle>Recent Platform Activity</CardTitle>
        <Link href="/audit" className="text-xs font-medium text-brand-600 hover:underline">View all activity</Link>
      </CardHeader>
      <CardContent>
        {events.length === 0 ? (
            ...
            {events.slice(0, 10).map((e) => (
              <li key={e.id} className="flex items-center gap-3 text-sm">
                ...
                <span className="min-w-0 flex-1 truncate text-surface-700">
                  ...
                  {e.actor?.name && <span className="text-surface-400"> · {e.actor.name}</span>}
                </span>
