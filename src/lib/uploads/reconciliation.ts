...
export async function generateUploadReconciliationReport(
  ...
  const [attempts, storedObjects, legacyPdfVersions] = await Promise.all([
    ...
    client.pdfVersion.findMany({
      where: { storedObjectId: null },
  ...
  for (const pdfVersion of legacyPdfVersions) {
    // Placeholder rows (no stored bytes) predate real generation; rows with a
    // recorded size hold real generated bytes and are expected to become
    // durable — the generated-document analogue of the PR-5C.3 gate metric.
    findings.push({
      category: pdfVersion.fileSize === null ? "LEGACY_PLACEHOLDER" : "GENERATED_VERSION_NOT_DURABLE",
      resourceType: "PDF_VERSION",
      resourceId: pdfVersion.id,
    })
  }
